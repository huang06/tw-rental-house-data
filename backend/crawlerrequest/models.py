...
from rental.models import BaseTimeSeries, JSONField, Vendor
...
class RequestTS(BaseTimeSeries):
    request_type = models.IntegerField(choices=[(tag, tag.value) for tag in RequestType])
    ...
    class Meta:
        db_table = 'request_ts'
        indexes = [models.Index(fields=['year', 'month', 'day', 'hour'])]
